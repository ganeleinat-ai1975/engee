...
const generateAdminEmailHtml = (order) => {
    ...
    return `
      <div style="font-family:Arial,sans-serif;direction:rtl;text-align:right;">
        ...
        <p><strong>אימייל:</strong> ${order.user_email}</p>
...
Deno.serve(async (req) => {
    try {
        ...
        if (!orderId) {
            return new Response(JSON.stringify({ success: false, error: "orderId is required" }), { status: 400 });
        }
        ...
        // === תיקון 1: מציאת כתובת המייל גם באורחים ===
        let targetEmail = order.user_email;
        
        // בדיקת שדות נוספים אם user_email ריק
        ...
        console.log(`Order #${order.order_number}: Processing email. Target found: ${targetEmail || 'NONE'}`);

        // === תיקון 2 ו-3: שליחה סדרתית ומוגנת ===
        
        // שלב א': שליחה ללקוח (אם יש מייל)
        if (targetEmail) {
            try {
                ...
                console.log(`Order #${order.order_number}: Customer email sent successfully.`);
            } catch (e) {
                // תופסים שגיאה כדי לא לעצור את המייל למנהל
                console.error(`Order #${order.order_number}: Failed to send customer email. Error:`, e.message);
            }
        } else {
             console.warn(`Order #${order.order_number}: Skipping customer email (No email address found).`);
        }

        // שלב ב': שליחה למנהל (מתבצעת תמיד, גם אם שלב א' נכשל)
        try {
            // מעדכנים את אובייקט ההזמנה עם המייל שמצאנו כדי שיופיע בפרטי המנהל
            const orderForAdmin = { ...order, user_email: targetEmail || "לא נמצא (אורח)" };
            const htmlToAdmin = generateAdminEmailHtml(orderForAdmin);
            ...
            console.log(`Order #${order.order_number}: Admin email sent successfully.`);
        } catch (e) {
            console.error(`Order #${order.order_number}: Failed to send ADMIN email. Error:`, e.message);
        }
        
        // עדכון סטטוס
        await base44.asServiceRole.entities.Order.update(order.id, { email_sent: true });
        
        return new Response(JSON.stringify({ success: true, message: "Emails processed" }), { status: 200 });

    } catch (error) {
        console.error('sendConfirmationEmails critical error:', error);
