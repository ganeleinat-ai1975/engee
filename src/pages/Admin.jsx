...
import {
  ...
  Save,
  HelpCircle, // Added for FAQ tab
} from "lucide-react";
import { Checkbox } from "@/components/ui/checkbox";
import { useForm } from 'react-hook-form';
import CouponsManager from "@/components/admin/CouponsManager";
import WorkshopSettings from "@/components/admin/WorkshopSettings"; // Import the new WorkshopSettings component
import MediaUploader from "@/components/MediaUploader"; // Import the new MediaUploader component
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 md:px-6 lg:px-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4 md:space-y-6">
          <div className="overflow-x-auto -mx-2 px-2">
            <TabsList className="inline-flex flex-nowrap md:grid md:grid-cols-4 lg:grid-cols-8 gap-1 md:gap-2 p-1 md:p-2 min-w-max md:min-w-0 md:w-full">
          ...
          {/* תוכן עמודים - Placeholder for new tab */}
          <TabsContent value="pages" className="space-y-6">
            <SettingsSection title="ניהול תוכן עמודים">
                <p className="text-gray-600">
                    כאן ניתן יהיה לנהל את התוכן עבור עמודים שונים באתר (לדוגמה: אודות, צור קשר, מדיניות פרטיות וכו').
                </p>
                <p className="text-gray-600">
                    הגדרות עמוד 'אודות' ו'צור קשר' נמצאות כרגע תחת 'הגדרות כלליות'.
                </p>
                {/* Example of how to use handleMediaUpload and handleRemoveMediaItem */}
                {/* This is just a placeholder to show the functionality */}
                <Card>
                    <CardHeader><CardTitle>מדיה לעמוד הבית</CardTitle></CardHeader>
                    <CardContent>
                        {/* Note: This is a simplified input, MediaUploader should be used for robustness */}
                        <input type="file" onChange={(e) => handleMediaUpload('home', e.target.files[0])} />
                        <div className="mt-4 space-y-2">
                            {siteSettings.page_media?.home?.map((item, index) => (
                                <div key={index} className="flex items-center gap-2 p-2 border rounded">
                                    {item.type === 'image' && <img src={item.file_url} className="w-16 h-16 object-cover" />}
                                    {item.type === 'video' && <video src={item.file_url} controls className="w-16 h-16 object-cover" />}
                                    <span>{item.file_url}</span>
                                    <Button size="sm" variant="destructive" onClick={() => handleRemoveMediaItem('home', index)}>Remove</Button>
                                </div>
                            ))}
                        </div>
                    </CardContent>
...
function ProductForm({ product, onSubmit, onCancel }) {
  const { register, handleSubmit, setValue, watch, reset } = useForm({
    defaultValues: {
      ...
      description_en: '',
      price: '',
      category: 'rings', // Default to 'rings'
      is_featured: false,
      stock_quantity: 100, // Add stock quantity
    }
  });
  ...
  const selectedCategory = watch('category'); // Watch category to conditionally show size options

  // הגדרת מידות לפי קטגוריה
  const getSizeOptions = (category) => {
    switch(category) {
      case 'rings':
        return Array.from({length: 16}, (_, i) => (48 + i).toString()); // Sizes 48-63
      case 'necklaces':
        return ['37', '42', '45', '50', '55']; // Standard necklace lengths in cm
      case 'bracelets':
        return ['15', '18']; // Standard bracelet lengths in cm
      default:
        return [];
    }
  };
  ...
  useEffect(() => {
    // When editing an existing product, populate the form
    if (product) {
      reset({
        name: product.name || '',
        ...
        stock_quantity: product.stock_quantity ?? 100, // Handle stock quantity for existing product
      });
      setCurrentImages(product.images || []);
      // Ensure available_sizes are filtered based on the product's category on load
      ...
      // Reset form for new product (when product prop becomes null)
      reset({
        name: '', name_en: '', description: '', description_en: '', price: '',
        category: 'rings', is_featured: false, stock_quantity: 100 // Default stock for new product
      });
      setCurrentImages([]);
      setAvailableSizes([]);
    }
  ...
  // עדכון מידות זמינות כאשר משנים קטגוריה
  useEffect(() => {
    const newSizeOptions = getSizeOptions(selectedCategory);
    // שמירה רק על מידות שקיימות בקטגוריה החדשה
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      // Direct use of UploadFile from Core
      const { file_url } = await UploadFile({ file });
  ...
  const internalSubmitHandler = async (data) => {
    ...
    try {
      const productData = {
        ...
        // הוספת available_sizes לכל הקטגוריות שיש להן מידות
        ...(currentSizeOptions.length > 0 && { available_sizes: availableSizes }),
      };

      await onSubmit(productData);

      // Reset form after successful submission only if not editing
      // If editing, the form will reset on product prop change
      if (!product) {
        reset();
        setCurrentImages([]);
        setAvailableSizes([]);
      ...
      // Parent onSubmit (handleProductSubmit in Admin) already handles toast messages
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                {isUploadingImage && <span className="text-sm text-gray-500">מעלה תמונה...</span>}
              </div>
            </div>
          </div>

          {/* מידות זמינות - לכל הקטגוריות שיש להן מידות */}
          {currentSizeOptions.length > 0 && (
            <div>
                ...
                {currentSizeOptions.map((size) => {
                  ...
                  return (
                    <button
                      key={size}
                      type="button"
                      onClick={() => toggleAvailableSize(size)}
              ...
              <p className="text-sm text-gray-600 mt-2">
                {selectedCategory === 'rings' && 'מידות ירוקות יהיו זמינות ללקוחות. מידות אפורות יוצגו כ"לא זמין".'}
                {selectedCategory === 'necklaces' && 'אורכים ירוקים יהיו זמינים ללקוחות. אורכים אפורים יוצגו כ"לא זמין".'}
                {selectedCategory === 'bracelets' && 'מידות ירוקות יהיו זמינות ללקוחות. מידות אפורות יוצגו כ"לא זמין".'}
              </p>
            </div>
          )}
...
function CategoryForm({ category, onSubmit, onCancel }) {
  ...
  // const [isUploadingImage, setIsUploadingImage] = useState(false); // This state is managed by MediaUploader internally
  const [isSubmitting, setIsSubmitting] = useState(false);

  const watchedName = watch('name');

  // Helper function for general slug generation
  ...
  // פונקציה להמרת שם עברי ל-slug באנגלית
  const generateSlugFromHebrewName = (hebrewName) => {
    ...
    return hebrewToEnglishMap[hebrewName] || generalSlugify(hebrewName);
  };

  // עדכון אוטומטי של ה-slug כאשר השם בעברית משתנה
  useEffect(() => {
    const newSlug = generateSlugFromHebrewName(watchedName);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{category ? 'עריכת קטגוריה' : 'הוספת קטגוריה חדשה'}</CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit(internalSubmitHandler)} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                {...register('name_en')}
                placeholder="For example: Rings"
              />
            </div>
          </div>

          <div>
            <Label>URL (slug) - אוטומטי</Label>
            <Input
              {...register('slug')}
              readOnly
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                className="rounded"
              />
              <Label htmlFor="is_active_category">קטגוריה פעילה</Label>
            </div>
          </div>

          <div className="flex gap-2">
            <Button type="submit" disabled={isSubmitting}>
              {isSubmitting ? 'שומר...' : (category ? 'עדכן קטגוריה' : 'הוסף קטגוריה')}
            </Button>
